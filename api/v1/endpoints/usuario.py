...
@router.get('/logado', response_model=UsuariosSchemaBase)
    ...
    async with db as session:
        try:
            ...
            raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE,
                                detail="Ja existe esse e-mail cadastrado")
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(),
    ...
    if not usuario:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail='Usuario incorreto!')
